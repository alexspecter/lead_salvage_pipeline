...
class Phase2Runner:
    ...
    def process(self, rows: List[LeadRow]) -> List[LeadRow]:
        self.logger.log_event(
            PHASE_2_SEMANTIC, "START", reason=f"Processing {len(rows)} rows"
        )
        ...
        if not ai_rows:
            self.logger.log_event(
                PHASE_2_SEMANTIC, "SKIP", reason="No rows require AI processing"
            )
            return rows
        ...
        if len(ai_rows) <= 200:
            ...
            self.logger.log_event(
                PHASE_2_SEMANTIC,
                "MODEL_SELECT",
                reason=f"Small batch ({len(ai_rows)} rows). Using 70B model for quality.",
            )
        else:
            self.logger.log_event(
                PHASE_2_SEMANTIC,
                "MODEL_SELECT",
                reason=f"Large batch ({len(ai_rows)} rows). Using 8B model for speed.",
            )
        ...
        while i < total_ai:
                ...
                if usage > 0.85:
                    ...
                    if new_size < current_chunk_size:
                        self.logger.log_event(
                            PHASE_2_SEMANTIC,
                            "MEMORY_PRESSURE",
                            reason=f"Usage {usage * 100:.1f}%. Reducing chunk size to {new_size}",
                        )
            ...
            try:
                ...
                self.logger.log_event(
                    PHASE_2_SEMANTIC,
                    "OOM_WARNING",
                    reason="Python MemoryError caught. Halving chunk size.",
                )
                ...
                if current_chunk_size == 1:
                    ...
                    self.logger.log_error(
                        PHASE_2_SEMANTIC,
                        "ROW_OOM",
                        Exception("Row too large for memory."),
                    )
                    row = ai_rows[i]
            ...
            try:
                            ...
                            if start != -1 and end != -1:
                                try:
                                    cleaned_data = json.loads(
                                        response_str[start : end + 1]
                                    )
                                except json.JSONDecodeError as e:
                                    parse_error = e
                            else:
                                parse_error = json.JSONDecodeError(
                                    "No JSON found", response_str, 0
                                )
                ...
                for key, value in cleaned_data.items():
                    ...
                    if key_lower in row["clean_data"] and row["clean_data"][key_lower]:
                        ...
                        is_useless = value is None or (
                            isinstance(value, str)
                            and (
                                not value.strip()
                                or value.strip().lower() == "not provided"
                            )
                        )
                    ...
                    if key_lower in preserved_fields and key_lower in row["clean_data"]:
                        ...
                        if key_lower == "phone":
                            ai_phone_empty = value is None or (
                ...
                row["confidence_score"] = (
                    0.8  # Arbitrary for now, could ask LLM for confidence
                )
