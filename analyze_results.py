...
def analyze(output_dir: str, run_id: str):
    ...
    if os.path.exists(log_file):
        ...
        enrichment = log[log['action'] == 'ROW_ENRICHED'] if 'action' in log.columns else []
        p2_resolved = log[log['action'] == 'ROW_RESOLVED'] if 'action' in log.columns else []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", "-o", default="user_output", help="Output directory to analyze")
    parser.add_argument("--run_id", "-r", help="Specific run ID to analyze")
    parser.add_argument("--latest", "-l", action="store_true", help="Auto-detect and analyze the latest run")
