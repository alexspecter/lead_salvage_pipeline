...
class DocxCleanerRunner:
    ...
    def run(self) -> str:
        ...
        self.logger.log_event(
            "DOCX_CLEANER", "START", reason=f"Processing {self.input_path}"
        )
        ...
        try:
            ...
            self.logger.log_event(
                "DOCX_CLEANER",
                "EXTRACT_COMPLETE",
                reason=f"Found {len(blocks)} content blocks",
            )
            ...
            for i in range(0, len(blocks), chunk_size):
                ...
                print(
                    f"Cleaned blocks {i + 1}-{min(i + chunk_size, len(blocks))} of {len(blocks)}"
                )
            ...
            self.logger.log_event(
                "DOCX_CLEANER", "RECONSTRUCT_COMPLETE", reason=f"Saved to {output_path}"
            )
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python -m lead_cleaner.docx_cleaner.runner <input.docx> [output.docx]"
        )
        sys.exit(1)
