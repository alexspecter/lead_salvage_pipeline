...
class TestNormalizers(unittest.TestCase):
    def test_email_normalizer(self):
        # Valid
        self.assertEqual(emails.normalize_email(" Test@Example.com ")["normalized_value"], "test@example.com")
        # Invalid
        self.assertEqual(emails.normalize_email("not_an_email")["field_status"], "INVALID")
        # Empty
        self.assertEqual(emails.normalize_email(None)["field_status"], "MISSING")

    def test_phone_normalizer(self):
        # Clean 10 digit
        self.assertEqual(phones.normalize_phone("1234567890")["normalized_value"], "123-456-7890")
        # Clean 11 digit with 1
        self.assertEqual(phones.normalize_phone("11234567890")["normalized_value"], "123-456-7890")
        # Invalid
        self.assertEqual(phones.normalize_phone("123")["field_status"], "INVALID")

    def test_name_normalizer(self):
        self.assertEqual(names.normalize_name("  john doe  ")["normalized_value"], "John Doe")
...
class TestDeduplication(unittest.TestCase):
    ...
    def test_deduplication(self):
        rows = [
            {"row_id": "1", "status": RowStatus.AI_REQUIRED, "clean_data": {"email": "a@b.com"}, "raw_data": {}},
            {"row_id": "2", "status": RowStatus.AI_REQUIRED, "clean_data": {"email": "a@b.com"}, "raw_data": {}}, # Duplicate
            {"row_id": "3", "status": RowStatus.AI_REQUIRED, "clean_data": {"email": "c@d.com"}, "raw_data": {}}
        ]
