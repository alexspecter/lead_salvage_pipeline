...
class Phase1Runner:
    ...
    def process(self, df: pd.DataFrame) -> List[LeadRow]:
        self.logger.log_event(
            PHASE_1_DETERMINISTIC, "START", reason=f"Processing {len(df)} rows"
        )
    ...
    def _detect_field_types(self, df: pd.DataFrame) -> Dict[str, Optional[str]]:
        ...
        for col in df.columns:
            ...
            if not detected_type:
                ...
                if inferred == FIELD_Phone and any(
                    x in col_lower for x in financial_keywords
                ):
                    inferred = None
    ...
    def _apply_normalizer(
        self, field_type: str, value: Any, original_field: str
        ...
        try:
            ...
            return {"normalized_value": None, "field_status": "ERROR", "reason": str(e)}
