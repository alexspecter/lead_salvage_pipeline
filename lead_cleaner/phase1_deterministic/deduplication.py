...
def detect_duplicates(rows: List[LeadRow], logger: PipelineLogger) -> List[LeadRow]:
    ...
    if not DEDUP_ENABLED:
        logger.log_event(
            "PHASE_1", "DEDUP_DISABLED", reason="Deduplication is disabled"
        )
        return rows
...
def _dedup_by_single_field(
    rows: List[LeadRow], logger: PipelineLogger, field_name: str, field_label: str
) -> List[LeadRow]:
    """
...
def _dedup_by_composite(
    rows: List[LeadRow], logger: PipelineLogger, fields: List[str]
) -> List[LeadRow]:
    """
...
def _dedup_by_all_fields(rows: List[LeadRow], logger: PipelineLogger) -> List[LeadRow]:
    """
