...
def calculate_confidence(row: LeadRow, expected_fields: dict) -> float:
    ...
    if job_val:
        ...
        has_buzzwords = bool(
            re.search(
                r"\b(ninja|guru|visionary|rockstar|wizard|mom of)\b",
                str(job_val),
                re.IGNORECASE,
            )
        )
...
def route_row(row: LeadRow, expected_fields: dict = None) -> LeadRow:
    ...
    if expected_fields is None:
        expected_fields = {
            FIELD_Email: True,
            FIELD_Phone: True,
        }  # Default to expecting all for safety
    ...
    # (Checked during expected fields logic below)
    ...
    # Fallback to Generic Mode logic (if enabled AND scoring failed)
        ...
        field_count = len(
            [v for v in row["clean_data"].values() if v is not None and v != ""]
        )
