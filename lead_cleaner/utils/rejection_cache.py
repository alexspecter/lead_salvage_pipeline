...
class RejectionCache:
    ...
    def add_column_rejection(self, column_name: str, reason: str):
        ...
        if self.logger:
            self.logger.log_event(
                "REJECTION_CACHE",
                "COLUMN_REJECTED",
                reason=f"Column '{column_name}' dropped: {reason}",
            )

    def add_value_rejection(
        self, row_id: str, field: str, original_value: Any, new_value: Any, reason: str
    ):
    ...
    def cache_rejected_rows(self, rejected_df: pd.DataFrame):
        ...
        if self.logger:
            self.logger.log_event(
                "REJECTION_CACHE",
                "ROWS_CACHED",
                reason=f"Cached {len(rejected_df)} rejected rows",
            )
    ...
    def log_security_rejection(self, filename: str, reason: str):
        ...
        if self.logger:
            self.logger.log_event(
                "REJECTION_CACHE",
                "FILE_REJECTED",
                reason=f"File '{filename}' rejected: {reason}",
            )
    ...
    def save(self):
        ...
        if not self.cache_entries:
            ...
            df = pd.DataFrame(
                columns=["run_id", "type", "item_id", "reason", "details"]
            )
        else:
        ...
        if self.logger:
            self.logger.log_event(
                "REJECTION_CACHE",
                "SAVED",
                reason=f"Rejection cache saved to {self.cache_file}",
            )
